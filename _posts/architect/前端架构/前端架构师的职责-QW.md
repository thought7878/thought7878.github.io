前端架构师（Frontend Architect）**远不止是“写代码最厉害的人”**，而是一个**技术战略家、系统设计师、质量守护者和团队赋能者**的综合体。其`核心职责`是**通过技术决策和流程设计，确保前端系统在业务规模增长时仍能保持高质量、可维护性和团队高效协作**。
...
* **核心任务：**  
  - 评估技术栈（框架/工具/式）与**业务目标**的匹配度（例：高 SEO 需求选 Next.js；内部工具选轻量级方案）  
...
* **关键动作：**  
  ✅ **开发体验优化：**  
    ...
    - 实现 **“一键 Mock”**：本地开发自动代理 API 请求到 Mock 服务。参考：[[实现一键 Mock：本地开发自动代理 API 请求到 Mock 服务]]、[[04 接口调试：Mock 工具如何快速进行接口调试？]]
  ✅ **组件库治理：**  
    - 用 Storybook 建立文档，强制组件通过 a11y 测试
    - 设计 **“组件演进协议”**（例：弃用 `Button` 的 `type` 属性时提供自动 codemod）  
  ✅ **自动化流水线：**  
    - PR 自动部署预览环境（Vercel/GitHub Pages）  。参考：[[21 容器方案：从构建到部署，容器化方案的优势有哪些？]]
    - 用 `changesets` 管理版本发布  
* **真实场景：**  
  > 新人入职第一天：  
