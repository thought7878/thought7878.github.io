...
> **传统的 SSR 会在服务器上为每个请求渲染完整的 HTML**。React 18 引入了`流式服务器端渲染`，**它会在生成 HTML 的同时将 HTML 分块发送给客户端，而不是等待整个渲染完成**。这**显著提升了 TTFB 和 LCP——用户几乎能立即看到页面框架**。
> 
...
> `React 服务器组件`：最大的变化可能是 React 服务器组件（RSC）的兴起。**RSC 允许你在服务器上提前渲染 UI 的一部分，而无需将相关的 JS 发送到客户端，*从而显著减小客户端 JS 打包文件的大小***。
> **容器/展示模式非常适合 RSC：** 所谓的“`容器`”（数据获取逻辑）可以是一个获取数据并将其作为 props 传递给展示式客户端组件的服务器组件。
