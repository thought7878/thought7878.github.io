...
讲座探讨了`质量属性`，也被称为`非功能性需求`。这些属性描述了软件系统的整体特性。质量属性衡量了系统在特定方面的表现，并且对软件架构有着直接的影响。讲座强调了设计具有适当质量属性的系统的重要性，以避免需要进行重大的重新设计。质量属性需要满足所有利益相关者的需求，并且必须是可以衡量和验证的。不过，并没有一种通用的架构能够同时满足所有质量属性，因为有些属性是相互矛盾的。因此，架构师必须根据具体情况来优先处理各项需求，从而设计出合适的系统。
...
讲座讨论了`系统约束`，这些是不可协商的决策，*限制了建筑师在设计软件架构时的自由度*。*系统约束分为三种类型：* 技术约束、业务约束和法律约束。`技术约束`包括由于现有合同、支持成本或客户请求而导致的编程语言、技术或平台的限制。`业务约束`是由业务团队做出的决策，要求建筑师在架构和实现方面做出牺牲，以符合业务目标。`法律约束`包括对在线服务施加某些限制的法规，例如美国的医疗信息 HIPAA 法规或欧盟的 GDPR。
...
本次讲座介绍了`负载均衡器`，它们是实现现实世界中**大规模系统的关键组件**。负载均衡器的**主要作用**是*将负载均匀分配给系统中的多台服务器*。负载均衡器**通过隐藏一组服务器来提供高可扩展性和可用性，从而实现水平扩展、自动扩展功能，并能够忽略那些失效或速度过慢的服务器**。负载均衡器在保持服务级别协议的同时，**提升了系统的性能和吞吐量**。讲座还介绍了*不同类型的负载均衡器*，包括 `DNS 负载均衡器`、`网络负载均衡器`、`硬件负载均衡器`以及`应用程序负载均衡器`，并分析了它们的优缺点。
...
讲座介绍了**异步架构的基本架构组件**——`消息代理`。首先，讲座探讨了*在哪些情况下，异步架构能够比同步通信提供更多的优势和更好的功能*。**同步通信的第一个缺点**是，相互之间进行通信的两个应用程序实例*都必须保持正常运行，以完成事务处理*。**第二个缺点**是，*系统中没有机制来应对突然增加的流量或负载*。`消息代理`可以**提供额外的功能，如消息路由、消息转换、消息验证，甚至负载均衡**。消息代理**是任何异步软件架构的基本组成部分**。
...
`API 网关`是一个在大规模系统中**用于解决扩展带来的复杂性的基础架构组件**和设计模式。它*作为一个 API 管理服务，对一组后端服务进行抽象和统一，简化外部 API*。网关遵循一种称为 API 组合的软件架构模式，允许它将后端服务的不同 API 组合成一个外部 API。**API 网关的好处**包括*系统内部变更的便捷性、增强的安全性和身份验证、通过请求路由提升系统性能、缓存静态内容以减少响应时间*。
...
这门讲座是**关于数据库**的，重点介绍*关系型数据库*。讲师解释 说，`关系型数据库`**以表格形式存储数据，每一行代表一个记录，所有记录通过预定义的一组列相互关联**。*正是所有记录之间的这种关系，使得这种类型的数据库被称为关系型数据库*。表中的每个记录都由一个称为`主键`的**唯一标识符来标识**，主键可以是表中一个或一组列的表示。**关系型数据库最大的优势之一**是它们**允许消除数据重复，这节省了存储空间，并直接转化为企业的成本节约**。
...
讲座介绍了`非关系型数据库`，也称为 `NoSQL 数据库`。这类数据库**解决了关系型数据库的局限性**，比如*对灵活模式的需求*以及*查询方面的挑战*。非关系型数据库旨在**实现更快速的查询操作**，并**支持更符合编程语言特性的数据结构**。不过，当模式具有灵活性时，非关系型数据库就**失去了分析记录的能力**。讲座还探讨了*三种主要类型的非关系型数据库*：**键/值存储、文档存储、图数据库**，并给出了它们的应用场景示例。
...
讲座讨论了**三种提高大规模系统中数据库性能、可用性和可扩展性的技术**。首先，`索引技术`通过将列与记录关联起来，从而加速数据的检索操作。其次，`数据库复制技术`可以将关键数据复制到不同的计算机上，从而实现高可用性和更好的性能。最后，分片技术可以将大型数据库拆分为多个较小的部分，从而提升可扩展性和分布性。
...
讲座中，作者以分布式数据库为背景*解释了 CAP 定理*。`CAP` 指的是**一致性、可用性、分区容忍性**。该定理指出，*在网络分区的情况下，分布式数据库无法同时保证一致性和可用性，因此必须选择其中之一*。讲座中*通过实例详细说明了该定理的实际应用情况*。
...
讲座讨论了非结构化数据的重要性，并给出了在哪些情况下需要使用这种数据的实际案例。`非结构化数据`，如**图片、视频、文档**等，**往往过于庞大，无法存储在传统的数据库中，因此需要可扩展的解决方案**。讲座介绍了两种解决方案：`分布式文件系统`和`对象存储`。这两种方案各有优缺点。`分布式文件系统`提供了**类似文件文件夹的结构**，**适用于机器学习和大数据分析**。而`对象存储`则**专为在互联网规模上存储非结构化数据而设计**，*通过增加更多的存储设备来实现可扩展性*。

[[5.可扩展的非结构化数据存储]]

#### 可扩展的非结构化数据存储解决方案——云和开源技术
...
探索`软件架构模式`作为**解决常见系统设计问题**的**可重复方案**。*了解这些模式为何重要*，以及*它们如何避免系统陷入混乱*，同时*实现安全的系统迁移*。
...
这篇讲座介绍了多层架构模式，并**重点介绍了该模式中最常见的一种变体**——`三层架构`。`多层架构`**将应用程序物理划分为多个层次，通过逻辑上的分离来限制每个层次的责任范围**。`三层架构`由`表示层`、`应用层`和`数据层`组成，**常用于基于 Web 的服务**中。这种架构**易于维护和扩展**；不过，其**缺点**在于*逻辑层往往呈现出单一整体的结构*。
