...
## Why we need a client-side architecture standard我们需要客户端架构标准的原因
...
### Model-View presenter 模型-视图-呈现者
...
### MVC & MVP are too genericMVC 和 MVP 过于泛化
...
### Tools used within the model模型中使用的工具
...
## We need a shared language to talk about client-side architecture我们需要一种共同的语言来讨论客户端架构
...
## We've already solved this problem我们已经解决了这个问题
...
## Client-side needs — testability, flexibility, and maintainability客户端需求——可测试性、灵活性和可维护性
...
### When to write unit tests 何时编写单元测试+
...
### When to write integration tests 何时编写集成测试+
