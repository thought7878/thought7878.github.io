...
展示组件存在于 Model-View-Presenter 的 View 部分边界内。它们全部的目的是：
...
### Presentation components are an implementation detail展示组件是实现细节
...
实现细节是帮助我们实现主要目标的低级细节。但它们不是我们的主要目标。如果我们主要目标是连接添加待办事项功能，那么界面中的按钮、样式和文本是实现该功能时的实现细节。

### Presentation components can be volatile展示组件可能是易变的
...
任何经常发生变化的事物都被认为是易变的。我们不断改变组件的外观和感觉，正是使它们如此的原因。
...
应对这种现象的一种方法是确定一套稳定的可重用组件（这些组件要么是你自己编写的，要么是从组件库中获取的），然后从这些组件创建你的视图。
...
### Should we include GraphQL queries in our presentation components?我们应该在我们的展示组件中包含 GraphQL 查询吗？
...
将 GraphQL 查询尽可能靠近展示组件是件好事。查询定义了数据需求。而且，如果需求发生变化，它们很可能需要一起修改，将它们放在一起可以减少在文件之间来回切换所累积的不必要的认知负担。
...
将你的查询放在组件中的一个潜在缺点是，现在如果你想要从 GraphQL 切换，你的组件就不是纯净的——它们与 GraphQL 耦合了。如果你想要切换传输层技术，你将不得不重构每个组件。
...
另一个潜在的缺点是，为了测试这些组件，你需要确保它们被包裹在一个模拟的 Apollo Client 提供者中。
...
我仍然建议将查询与组件结合起来。在我看来，获得卓越的开发体验是值得冒险的，即完全采用 GraphQL，并在未来决定想要改变。
...
关于查询性能的说明：对于像上面展示的这样大量针对超级特定数据块的查询是没问题的。使用 Apollo Client 时，Apollo 会处理检查数据是否已经被缓存的复杂逻辑，如果没有缓存——它会发起请求来获取数据。

### What to test in presentation components在演示组件中要测试什么
...
单元测试实现细节通常是徒劳的——尤其是对于易变的事物。测试一个按钮是蓝色还是绿色对我们并没有太多好处。相反，在测试展示组件时，我们想要针对 UI 逻辑进行测试。

To demonstrate what I mean, here's a bland, basic presentation component.  
为了说明我的意思，这里有一个平淡无奇的基础展示组件。
...
之前展示的两个 `Todo` 组件之间的主要区别在于，第二个 `Todo` 组件包含了 UI 逻辑，而第一个则没有。

### UI logic is view behaviorUI 逻辑就是视图行为
...
一个组件具有 UI 逻辑，当它表现出行为时。决定显示什么内容的条件，或某些用户事件何时被调用优先于其他事件，是一种视图行为（UI 逻辑）。

Here's a conditional example from the previous code sample **determining what to show**.  
这是来自上一个代码示例的一个条件性示例，用于确定要显示什么内容。
...
在 Jed Watson 于 2019 年 GraphQL Summit 上的演讲《关于状态的小册子》中，他描述了在构建 Web 应用时涉及到的五种不同类型的状态： `local (component)` 、 `shared (global)` 、 `remote (global)` 、 `meta` 和 `router` 。
...
- **Explanations of the five types of state  
	...
	- `local (component)`: State that belongs to a single component. Can also be thought about as UI state. UI state can be extracted from a presentation component into a React hook. **Note: we're about to do this**.  
		`local (component)`: 属于单个组件的状态。也可以理解为 UI 状态。UI 状态可以从展示组件中提取到 React 钩子中。注意：我们即将这样做。
	- `shared (global)`: As soon as some state belongs to more than one component, it's *shared* global state. Components shouldn't need to know about each other (a header shouldn't need to know about a todo).  
		`shared (global)`: 当某个状态属于多个组件时，它就是共享的全局状态。组件之间不需要相互了解（头部组件不需要了解待办事项组件）。
	- `remote (global)`: The state that exists behind APIs in services. When we make `queries` for remote state, we hold onto a local copy of it accessible from a global scope.  
		`remote (global)`: 服务中 API 背后存在的状态。当我们为远程状态创建 `queries` 时，我们会保留一个可在全局范围内访问的本地副本。
	- `meta`: Meta state refers to state *about* state. The best example of this is the `loading` async states that tell us the progress of our network requests.  
		`meta`: 元状态指的是关于状态的状态。最好的例子就是 `loading` 异步状态，它们告诉我们网络请求的进度。
...
这个状态， `local (component)` 状态，属于单个组件。你可以称这个为 UI 状态。它的目的是保存帮助单个组件完成其工作的数据。
...
为了更好地查看其外观，让我们从这个基于类的组件中提取所有 UI 状态，并将其重构为一个函数式组件和一个 React 钩子。
...
### UI logic is what we actually try to test within componentsUI 逻辑是我们实际上在组件中尝试测试的内容
...
由于 UI 逻辑是行为，实际上这是我们希望在集成测试中测试的内容。行为。你也可以编写单元测试，但如果组件逻辑很简单，这可能就没什么意义了。将组件和 UI 逻辑一起进行集成测试可能更有价值，并且能让你更有信心地确信功能运行正常。

## Container/controller 容器/控制器
...
这并非新事物。控制器/呈现者的定义，从模型-视图-呈现者模式开始，就做出了这种区分。

### Do we really need container components?我们真的需要容器组件吗？
...
> 我发现\[容器组件\]有用的主要原因是它让我能将复杂的带状态逻辑与其他组件方面分离。Hooks 让我能不通过任意划分实现同样的事情。
...
此前，React 开发者被建议将数据和逻辑放在容器组件中，并编写决定"事物如何运作"的代码。但这违反了容器/展示器被指出的责任规则。
...
仅仅因为我们知道在 React Hooks 中放置有状态数据和行为，并不意味着我们移除了容器组件所解决的问题。
...
我们仍然需要配置响应性，有时使用 Redux，有时使用 Apollo Client 或其他工具，并且我们仍然需要一个结构作为粘合剂，知道要为页面启用的功能加载哪些组件。

### Container components are pages容器组件是页面

In the following React Router example, we have three main *pages:* home, about, and dashboard.  
在以下 React Router 示例中，我们有三个主要页面：主页、关于页和仪表盘页。
...
- is responsible for enable a variable number of features (remember, a feature is a `command` or `query`)  
	负责启用一个可变数量的功能（记住，一个功能是一个 `command` 或 `query` ）
...
- knows about `shared` reactive state, and *sometimes* connects it to presentational components that need it  
	知道 `shared` 反应式状态，有时会将其连接到需要它的展示组件
...
容器组件是特定页面的顶层模块，用于启用所有功能。在 Gatsby.js 中，我们称它们为页面组件。由于所有客户端架构都自然地遵循模型-视图-呈现者模式，因此我们不太可能完全废除呈现者（容器）。
...
为了证明我的观点，这里有一个在 React hooks 世界中的容器组件。它看起来可能不起眼，但请注意它承担了容器组件的两个职责。
...
有某物负责知道如何连接到响应式模型，以及如何处理来自展示组件的事件。这就是容器。**
...
当然，你可以将所有东西都称为组件，但那样我们为之奋斗的明确沟通和责任界定就会丢失。

### Container components contain no functionality容器组件不包含任何功能
...
当你点击提交来“添加待办事项”时，你会直接跳转到 GraphQL `mutation` 吗？你会执行任何验证逻辑吗？是否有任何规则需要强制执行？
...
假设你有一个名为 `createTodoIfNotExists` 的 `command` 。负责交互层的任何结构都包含帮助你决定“是否继续执行这个操作”的代码。
...
这里有一个 Redux Thunk 的例子，有时我们需要从某种 `global` 状态（可能存储在某个 store 中）中获取信息来做出决策。
...
### It contains your application's operations它包含您应用程序的操作
...
### Other ways to implement the model实现模型的其它方法
...
### There are usually several layers通常有数层
...
## 🚡 Networking & data fetching (infrastructure)🚡 网络与数据获取（基础设施）
...
## 🗄️ State management & storage (infrastructure)🗄️ 状态管理 & 存储（基础设施）
...
### State management and networking are often solved together状态管理和网络通常一起解决
...
### Mixture of remote and local state远程和本地状态的混合
