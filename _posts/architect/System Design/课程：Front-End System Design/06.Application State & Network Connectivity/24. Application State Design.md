...
- 需搜索操作则构建索引，
- 存储大量数据时转移到硬盘存储，
- 根据数据读写频率和大小选择合适存储类型，频繁读取的小型数据用同步存储（本地存储或会话存储），提升应用响应速度

- 你需要了解数据的规模。
- 始终*从数据的结构*化方式入手，并**使用范式来优化访问成本**。
- 如果你打算在应用程序中**执行搜索操作**，*使用索引*。
- 如果你认为**需要在状态中存储大量数据**，那么就将数据**转移到硬盘上**。对我们开发者来说，这基本上就是我们可以使用的空闲空间。
- **选择合适的数据存储**。这将提高你的应用程序的响应速度。而且你还需要留意所使用的存储类型。如果需要频繁读取数据，请利用数据属性，使用异步存储。对于小型数据，则使用本地存储或会话存储。
...
而且它还是非阻塞的。所以这是同步的，意思是在这种情况下我们不会阻塞UI线程。  
...
叶夫根尼·雷：所以关于应用状态设计的总结是，*首先，你需要了解自己的规模。始终从数据的结构化方式入手，并使用范式来优化访问成本。*
...
如果你打算在应用程序中执行搜索操作，请使用索引。如果你认为需要在状态中存储大量数据，那么就将数据转移到硬盘上。对我们开发者来说，这基本上就是我们可以使用的空闲空间。
...
这将提高你的应用程序的响应速度。而且你还需要留意所使用的存储类型。如果需要频繁读取数据，请利用数据属性，使用异步存储。对于小型数据，则使用本地存储或会话存储。
