...
- 更新根节点，使用布局环境并从Root块格式上下文*创建新块*（因设置display: inline-block）。
...
[03:33]
- 更新DOM树和渲染对象树，
- *创建新渲染层*且*该渲染层分配到、提升到不同图形层*（因启动新3D变换），
- 同时*创建新堆叠上下文*，将div标签启动的格式上下文放入其中。最终形成的堆叠上下文在浏览器中，根上下文之上会有覆盖的堆叠上下文且位于顶部。

![[_posts/architect/System Design/课程：Front-End System Design/02.核心基础知识/media/74328e158b284bcb2c0ce4ba1c5fdeff_MD5.jpeg]]

## 回顾课程内容起点
[04:25]
课程最初从盒模型的边距和填充等基础概念开始，**探讨绘画层次及其协同工作方式**。
