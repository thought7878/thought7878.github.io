...
叶夫根尼回顾了DOM API中的全局对象和类层次结构。文中还讨论了查询方法，并对它们的时间复杂度、内存成本和算法进行了比较。总体而言，getElementById在空间和时间复杂度方面表现最佳。querySelector和querySelectorAll的性能稍差，但由于缓存和浏览器优化，重复运行时性能会有所提升。
