...
source: "https://www.aihero.dev/cohorts/ai-coding-for-real-engineers-m0k0w"
author:
published:
created: 2026-09-26
...
这是一期为期两周的培训课程，专为那些希望在实际工作中使用人工智能编程工具的开发者们设计。课程内容涵盖情境分析、计划制定、任务执行与监控、反馈机制的建立、自动化处理流程的运用，以及人工干预的必要环节。


**2-week learning cohort • Starting June 1 • $995  
2 周学习小组 • 6 月 1 日开始 • 995 美元**
...
> 很难表达过去两个月由于 AI 编程发生了多大变化：不是逐渐地、随着时间的推移以“按部就班”的方式，而是具体到上个月的 12 月。
> 
> ![Andrej Karpathy](https://res.cloudinary.com/total-typescript/image/upload/c_limit,w_96/f_auto/q_auto/v1773151907/cohorts/cohort-hx0zg/kbvhhpasb83qwttcu44k?_a=BAVMn6DY0) Andrej Karpathy 安德烈·卡帕西
...
通过每天使用人工智能来编写软件，我已经学到了很多东西。这改变了我的许多看法，也显著提升了我对现代软件工程的认知。
...
像 Claude Code 这样的工具可以是你的最佳朋友，也可能是你最糟糕的队友。**

It's all in how you use it.  
这取决于你如何使用它。

# 大多数开发者以错误的方式开始使用 AI 进行编码
...
根据我的经验，大多数开发者在使用 AI 编码工具时犯有两个主要错误：

01

**Error #1:错误 #1：** They delegate everything.他们把所有事情都委托出去。

02

**Error #2:错误#2：** They delegate nothing.他们没有任何委托事项。

**When devs make the first error, they're excited.  
...
你需要设计你自己的主动、高效的中庸之道。既不是 YOLO 也不是 OH NO。
...
在生产环境中——尤其是针对已有的代码基而言——需要采用专业的方法来进行操作。
...
最后一个问题确实值得我们高度重视，因为失去这种感知不仅仅是一种情绪反应。这实际上是一种真正的认知损失，会导致理解能力的丧失、团队的混乱，以及一种足以对开发者及其团队造成实际（甚至可能是长期性）影响的压力状态。
...
不过，这些问题并非新出现的，尽管新工具的出现使得这些问题变得更加严重了。**
...
真正的工程师对其工具了如指掌：知道工具的用途，了解其擅长和出色的方面，明白其不足和缺陷，以及如何使其损坏。
...
作为工程师，你应该知道，虽然人工智能是一个新颖且不寻常的工具……但它仍然是一个工具。你必须有意地将其融入你的专业软件开发过程中。
...
如果你在今天的项目开发过程中不运用这些技能，或者不定期地使用这些技能，那么现在就是你学习如何运用它们的好时机了。
...
你的工程技能决定了项目是成功还是失败，是能够提升生产力还是引发技术债务，是编写可靠的代码还是编写可能带来全局问题的代码。

---

# 现在：像真正的工程师一样学习如何使用人工智能编程技术
...
从整理我的想法，到实际的录制和编辑过程。我知道，合适的软件能让我更加专注于打造出色的学习体验。
...
但这是我的生计来源。这不可能只是一个随意的、即兴的项目而已。我需要的是一款可靠且专业的工具，而且我知道，花费这么多时间来做这件事并不值得。

So for multiple years, that tool remained a pipe dream, with many aborted attempts.  
因此，在多年时间里，那个想法一直只是个幻想而已，有许多尝试都失败了。
...
不过，自从我开始使用 Claude Code 和 Ralph 循环工具后，我已经提交了超过 1,000 次提交，还创建并关闭了超过 500 个属于自己的问题。如今，我的专业级课程创建工具包含了许多我从未想象到的功能：

- a full in-browser video editor  
	一个功能齐全的浏览器视频编辑器
- AI writing assistant with 14 unique modes  
	具有 14 种独特模式的 AI 写作助手
- Dropbox/CMS/YouTube/X/LinkedIn sync  
	Dropbox/CMS/YouTube/X/LinkedIn 同步

It’s all powered by TypeScript (obviously), and backed by the amazing Effect.ts library.  
所有功能都由 TypeScript 驱动，并且得到了出色的 Effect.ts 库的支持。
...
你会学习 Claude Code 的工作原理，它能做什么和不能做什么，如何准备你的工作，用高效但可靠的规则管理和引导它，让你的项目按计划进行并符合规格，测试输出，并在每一步确保质量。
...
你会学习如何将 Claude Code 融入实际的生产环境，其中已有代码库和流程。
...
你将体验到正确使用像 Claude Code 这样强大的工具如何增强你的能力并加深你对代码库的理解。

**In other words: You'll go from vibe coder to AI Hero.  
换句话说：你会从感觉型的程序员转变为 AI 英雄。**

Before: Vibe Coder 之前：Vibe Coder

After: AI Hero 之后：AI 英雄
...
设计结构部分和指导系统，但让人工智能负责具体的实现工作。

AI you need to hand-hold  
你需要引导人工智能的发展

AI worth delegating to 适合委托处理的 AI 任务
...
所有这些基本原则同样适用于其他人工智能编程工具——不过，我选择使用 Claude Code，因为对我来说，它是目前最优秀的工具。*
...
# 加入由真实工程师主导的 AI 编程培训课程，从 6 月 1 日星期一开始学习吧
...
每个周一，你都将完全访问该周的所有课程内容，可以自行调节学习速度和安排时间。你可以快速完成课程，也可以慢慢学习；如果需要进一步理解某个知识点，可以随时回看相关内容，并带着问题去参加答疑时间！
...
为了帮助你在第一天就能顺利开始学习，一旦你注册后，你将立即获得我专门为这门课程设计的一系列课程资料。这些资料将帮助你快速了解 Claude Code 的相关内容。
...
如果你刚刚开始使用 Claude Code，那么这些内容将有助于你熟悉 CLI 编程助手中的关键术语和功能。而如果你已经熟悉了这些工具，那么这些内容也会让你轻松回顾一下相关知识！
...
总的来说，所有这些技术都能帮助你的编码助手保持在“智能区域”中，从而写出优秀的代码；同时避免进入“愚蠢区域”，避免陷入无意义的循环之中。
...
我们将用办公时间结束第一周。带上你的问题，获得直接答案。无法参加直播？提前提交你的问题，并获取录音和文本。


## 第 2 周：反馈循环、离线代理与人在回路模式
...
第 2 周的内容建立在第 1 周所学的基础上，介绍了使 AI 辅助软件工程既值得信赖又可靠的系统。
...
首先，你会了解到为什么在处理代理时，绿色的 CI 管道是不可协商的，并学习哪些类型的测试属于（或不属于）你代理的红色-绿色-重构工作流。你会构建一个自定义的“技能”，将你团队的质量标准编码到每个任务中，以及预提交钩子，为你的代码质量添加一个确定性安全网！
...
然后，自动化你工程工作流程的关键： “拉尔夫·威格姆” 循环，一个看似简单却极其强大的模式，让你的代理能够自主构建整个功能到你的应用中。
...
你将学习和实践在两种模式下运行拉尔夫：首先，使用“人在回路中”模式（提示：你就是那个人）。人在回路中模式在学习和实践时为你提供可见性和控制。
...
那么，我们就不再使用辅助工具了，为 Claude Code 创造一个安全的开发环境，让它完全独立地工作——当然，在你不在场的时候。从 GitHub 的问题报告中选取需要修复的缺陷，分配优先级，然后将更改提交到相应的分支中。只要稍加练习，你就能让 Claude Code 在你需要处理其他重要事务时继续运行了。

**By the end of week two, you'll have a clear sense of when to stay in the loop vs. go AFK autonomous, including:  
到了第二周结束时，你将清楚地知道何时应该保持在线状态，何时可以选择离线自主学习。包括：**

- how and when to run research phases that save future sessions hours of work  
	如何以及何时进行研究阶段的工作，以节省后续几小时的时间。
- the power of flexible kanban boards instead of upfront plans  
	与传统的固定计划相比，灵活的看板管理方式的优势在于其灵活性。
- and maybe most importantly, how to make sure that your codebase is prepared for AI agents to actually navigate it effectively.  
	或许最重要的是，如何确保你的代码库能够适应人工智能代理的有效运作。
...
与 Matt 的第二次办公时间交流。同样的形式，但会提出更深入的问题。到现在为止，你已经有了丰富的实践经验，因此我们可以帮助你将实践中的知识与实际工作中的代码编写联系起来！
...
这门课程专注于 Claude Code CLI 技术，正是通过学习和实践这门技术，我取得了出色的成果。因此，这对我来说也是教授你们的最佳方式！
...
不过，一旦你掌握了这些技能，你就可以在几乎任何 CLI 编码辅助工具中使用相同的技术和知识，无论面对何种前沿人工智能模型都能轻松应对。
...
你不必遵循每周两次的教学安排，而是可以在每个周一接受一周内的全部课程。这样你就可以提前开始学习，或者选择较慢的学习速度。
...
您将有机会在 3 天的时间里参加 6 次在线辅导课程——这些课程的时间安排不同，以便您能尽可能多地获得我亲自提供的帮助。如果您无法参加，请提前告知；所有的辅导录像和记录都会为您保留下来。
...
当然，你将被邀请加入我的私人 AI 英雄 Discord，为你提供一个专属频道，让你与同行学员一起交流学习。

This way, you get the best of sync and async courses in one great experience.  
这样一来，你将获得同步和异步课程的最佳体验。


# 包含内容
What's included 

For $995 you'll learn everything you need to use AI coding like a real engineer.  
只需 995 美元，你将学会如何像真正的工程师一样使用 AI 编程。
...
- **Live Office Hours with Matt**. Live Q&A sessions on Monday June 1, Monday June 8, and Friday June 12, with two time-zone friendly options each office hours day. Submit questions in advance if you can't join live.  
	与 Matt 一起实时办公时间交流吧。6 月 1 日星期一、6 月 8 日星期一以及 6 月 12 日星期五，我们将举办实时问答活动。每个办公地点在办公时间期间都有两种适合不同时区的提问方式。如果无法参与实时交流，请提前提交问题即可。
- **All recordings and transcripts**. Every office hours session, captured and available for replay.  
	所有的录音和文字记录。每个办公时间进行的会议都被记录下来，可以重新播放。
- **Exclusive Discord access**. Learn alongside your fellow students, ask questions, share wins, and soak up the energy of a group of smart folks figuring things out together.  
	专属的 Discord 访问权限。可以与同学一起学习，提出问题，分享经验，并融入一群共同探索知识的聪明人们的群体之中，感受他们的活力与智慧。
...
这里列出的所有内容都是您可以保留的，因此您可以随时再次访问这些内容作为参考或复习之用，或者通过进一步的练习来深化对所学知识的理解，尤其是那些在课程中未能充分掌握的内容。
...
今天就注册吧，获取那些用于实现人工智能编程工具的技能、工作流程以及信心，让这些工具真正发挥出应有的作用——成为推动实际工程项目的强大力量。
...
你可以使用任何你喜欢的工具来学习——这些工程原理同样适用于 Copilot、Cursor、Windsurf 等工具。Matt 使用 Claude Code 进行教学。我们建议在学习课程时使用 Claude 工具，然后将其学到的知识应用到你喜欢的工具上。
...
是的——所有的课程、练习资料、代码仓库、办公时间录制的课程视频以及讲义都归您使用。

## 如果我要无法参加现场活动，或者存在时区问题而无法参加呢？
...
我们的办公时间安排在早晨和晚上（按照 Matt 的时区设定）。所有课程都会进行录制，并附有讲稿。您可以提前提出问题。每周的周一所有课程都会停止更新，您可以根据自己的节奏进行学习。

## 你的退款政策是什么？
What's your refund policy?

30-day refund policy. Contact [team@aihero.dev](mailto:team@aihero.dev).  
30 天退款政策。请联系 team@aihero.dev。

## 还有其他问题吗？
More questions? 

For pricing, PPP, team seats, invoices, gifting, and platform questions, see [aihero.dev/faq](https://www.aihero.dev/faq). Or email [team@aihero.dev](mailto:team@aihero.dev).  
关于定价、PPP、团队席位、发票、赠送和平台问题，请查看 aihero.dev/faq。或发送邮件至 team@aihero.dev。
...
### 5. 为课程设置 Claude 工具
Setting Up Claude For The Course  
