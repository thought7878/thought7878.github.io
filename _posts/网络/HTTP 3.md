**HTTP/3** 是 HTTP 协议的最新版本，于 2022 年正式发布（RFC 9114）。它基于 **QUIC（Quick UDP Internet Connections）协议**，旨在解决 HTTP/2 中仍然存在的性能瓶颈问题，特别是在网络不稳定或高延迟环境下的表现。HTTP/3 的核心目标是提高连接建立速度、减少队头阻塞问题，并增强传输的可靠性。
