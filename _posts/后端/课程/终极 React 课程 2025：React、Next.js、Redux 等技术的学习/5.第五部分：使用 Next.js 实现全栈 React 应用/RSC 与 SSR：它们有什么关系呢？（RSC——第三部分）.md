...
- **最大的概念误区**：很多开发者*误以为 RSC 协议中的 "Server/Client" 等同于传统意义上的 "Web 服务器/浏览器"*。
    - `React Server`：只是一个**执行环境**，指代 *“非浏览器的计算机环境”*。它*不一定是运行中的 Web 服务器*，理论上*也可以是构建时的本地环境*（用于静态站点生成）。
    - `React Client`：同样是一个**执行环境**，指代 *“消费渲染结果的环境”*。**在 SSR 场景下**，这个“客户端”环境其实是*在 Web 服务器上运行的*，其任务是*将组件渲染为 HTML 而不是 DOM 节点*。

## 2. Next.js的**初始渲染流程**（结合 RSC/SSR 初始渲染）
...
- `发送的内容`：除了 HTML，服务器*还会发送 **React、JS 代码块** (chunks)、**RSC Payload***。
- `RSC Payload 的作用`：包含已渲染的 Server Components 数据、传递给 Client Components 的 Props、Client Components 代码的 URL。**它让 React 在客户端拥有完整的组件树**视图，**以便后续更新时能保留 UI 状态**。
