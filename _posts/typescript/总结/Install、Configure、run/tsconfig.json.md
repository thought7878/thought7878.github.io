...
| 选项                             | 默认值                  | 说明                                           |
| ------------------------------ | -------------------- | -------------------------------------------- |
| **`strict`**                   | `false`              | 启用所有严格检查的元选项（*建议始终设为* `true`），*帮助在编译时捕获潜在错误* |
| `alwaysStrict`                 |                      | 对所有代码（包括全局代码）启用严格模式                          |
...
| `esModuleInterop`  |     | 解决 CommonJS/ESM 互操作问题，*允许 CommonJS 模块通过import导入*（需配合allowSyntheticDefaultImports）。`"esModuleInterop": true` |
| 模块解析：              |     |                                                                                                             |
| `moduleResolution` |     | 模块解析策略: Node/Classic/Node16/NodeNext                                                                        |
| `baseUrl`          |     | 相对路径的基准目录（与`paths`配合使用）                                                                                     |
| `paths`            |     | 路径映射（如`"@components/*": ["src/components/*"]`），*用于简化导入语句*。需配合打包工具                                           |
|                    |     |                                                                                                             |
|                    |     |                                                                                                             |
|                    |     |                                                                                                             |
...
| ------------ | --- | -------------------------------------------------------------- |
| types        |     | 仅包含指定的全局类型                                                     |
| typeRoots    |     | 自定义类型声明目录，指定类型声明文件的目录（如`["./types", "./node_modules/@types"]`） |
| skipLibCheck |     | 跳过对依赖项的类型检查，提升编译性能                                             |
|              |     |                                                                |
|              |     |                                                                |
|              |     |                                                                |
...
```json
{
  ...
  "typeRoots": ["./typings"],      // 自定义类型声明目录，指定类型声明文件的目录（如`["./types", "./node_modules/@types"]`）
