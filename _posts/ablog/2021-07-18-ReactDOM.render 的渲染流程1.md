...
#### 创建当前 Fiber 节点的 update 对象，并将其入队 
...
前面曾提到过，**performSyncWorkOnRoot 是 render 阶段的起点**，**render 阶段的任务**就是**完成 Fiber 树的构建，它是整个渲染链路中最核心的一环**。*在异步渲染的模式下，render 阶段应该是一个可打断的异步过程*（下一讲我们就将针对 render 过程作详细的逻辑拆解）。
