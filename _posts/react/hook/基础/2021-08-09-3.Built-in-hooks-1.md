...
useEffect ，顾名思义，用于执行一段副作用。什么是副作用呢?通常来说，**副作用**是指**一段和当前执行结果无关的代码**。比如说要<u>修改函数外部的某个变量，要发起一个请求</u>（影响下一次UI），等等。**也就是说，useEffect的callback函数是在本次函数执行之后才执行的。在函数组件的当次执行过程中，useEffect 中代码的执行是不影响本次渲染出来的 UI 的**。
