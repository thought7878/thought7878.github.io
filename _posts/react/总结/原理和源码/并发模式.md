React 的 **并发模式（Concurrent Mode）** 是 React 16 引入的一种全新的渲染模式，旨在通过*可中断的渲染*和*任务优先级管理*，提升应用的性能和用户体验。并发模式的**核心目标**是让 React 能够更灵活地处理高优先级任务（如用户交互）和低优先级任务（如后台数据加载），*从而避免页面卡顿或掉帧*。
