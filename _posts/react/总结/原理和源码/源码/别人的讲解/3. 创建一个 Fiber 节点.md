...
**React 在*首次渲染*过程中，根据 createElement() 返回的 ReactElement，调用createFiberFromElement()，生成 Fiber 节点，进而生成 Fiber 树。**
...
**根据已有 Fiber 节点，创建新的 Fiber 节点，应用场景**：_首次渲染后的第一次更新_，此时内存中只有一棵 Fiber 树。current Fiber 节点还没有 alternate 属性，就根据 current Fiber 节点*创建一个新的副本 Fiber 节点*。
...
```js
/**
 * React每次更新时，都会调用这个函数，根据老的Fiber节点current，创建新的workInProgress 节点
 *
...
export function createWorkInProgress(current, pendingProps) {
  ...
  if (workInProgress === null) {
    ...
    // 场景：第二次及以后的更新，渲染时，复用已有的 workInProgress Fiber 节点，更新其 pendingProps
