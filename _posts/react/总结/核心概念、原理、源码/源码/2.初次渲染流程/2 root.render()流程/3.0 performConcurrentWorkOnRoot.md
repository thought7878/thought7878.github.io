...
该函数的**主要职责包括：**
1. **选择渲染策略**：根据情况决定使用`并发渲染/时间切片(concurrent)`还是`同步渲染`
2. **完成渲染**：*将完成的工作赋值给root并执行提交，开始commit阶段*
3. **安排后续工作**：*确保后续任务被正确调度*
4. 清理状态：重置事件时间和过渡通道
5. 验证执行上下文：确保没有其他渲染或提交正在进行
6. 处理副作用：在开始工作前先处理任何待处理的被动effect
7. 确定工作优先级：获取下一个需要处理的lane(s)
8. 错误处理：如果渲染出错，尝试恢复
9. 一致性检查：验证渲染结果与外部store的一致性
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  // ！！！根据是否需要时间切片选择不同的渲染方式，开始render阶段
  ...
  if (exitStatus !== RootInProgress) {
    ...
    if (exitStatus === RootDidNotComplete) {
      ...
      // ！！！渲染完成，现在我们有一棵一致的树。下一步是提交它，commit阶段
