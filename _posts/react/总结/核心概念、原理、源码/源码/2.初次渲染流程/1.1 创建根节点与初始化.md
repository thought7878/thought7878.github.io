...
export function createRoot(
  ...
  // 创建根节点的内部实现（！！！18.2.0不是这个，是createContainer）
...
export function createFiberRoot(
  ...
  // 2. 创建Fiber树的根节点rootFiber（根Fiber节点）
  ...
  // 4. 初始化rootFiber.updateQueue更新队列
...
```
**功能**：
- 创建FiberRoot（FiberRootNode对象），**存储整个应用的元数据**
- **创建Fiber树的根节点**（根Fiber节点(host root fiber)）
- **建立FiberRoot与Fiber树根节点的双向引用**
- *初始化更新队列*
- 配置并发模式相关参数
...
function FiberRootNode(containerInfo, tag, hydrate) {
  this.tag = tag; // 标识是ConcurrentRoot=1（LegacyRoot=0）
  this.containerInfo = containerInfo; // DOM容器
  this.pendingChildren = null;
  this.current = null; // 指向当前Fiber树的根节点
  this.pingCache = null;
...
### (0) jsx()、jsxDEV()
`packages/react/src/jsx/ReactJSXElement.js`、`packages/react/src/ReactElement.js`

参考：[[_posts/react/总结/核心概念、原理、源码/源码/2.初次渲染流程/1 createRoot()流程/jsx]]

解析 render 参数 <App />，将其转换成虚拟 DOM 对象
<App /> 为原始代码，经 babel 编译后的代码形如：
- 生产环境：jsx(App, {})
- 开发环境：jsxWithValidation(App, {})
最终都是调用 ReactElement 函数返回一个标识虚拟 DOM 的对象
只不过开发环境比生产环境多些额外校验，不满足时控制台给出错误提示
...
- 获取内部FiberRoot引用
- 调用updateContainer函数**启动渲染**
- 处理unmounted状态异常


### (2) updateContainer 函数
参考：[[2.0 updateContainer]]、[[2.1 requestUpdateLane()]]、[[2.2 createUpdate]]、[[createUpdate()]]、[[2.3 enqueueUpdate]]

**源码位置**：`packages/react-reconciler/src/ReactFiberReconciler.js`、`packages/react-reconciler/src/ReactFiberReconciler.new.js`
```javascript
export function updateContainer(
  element: ReactNodeList, // ReactElement，jsx()的返回值
  container: OpaqueRoot, // FiberRoot
  parentComponent: ?React$Component<any, any>,
  callback: ?Function,
): Lane {
  
  // 1. 获取事件时间戳。performance.now()
  const eventTime = requestEventTime();
  
  // 2. 获取更新优先级(车道)；获取root fiber
  ...
  // 5. 将 update 对象加入 rootFiber 的 updateQueue 队列
...
检查嵌套更新深度：
![[_posts/react/总结/核心概念、原理、源码/源码/2.初次渲染流程/media/3a03f377b3c89a4359de8e25e93f52d4_MD5.webp]]
