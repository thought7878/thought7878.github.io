---
title: React技术揭秘
source: https://react.iamkasong.com/state/prepare.html
author:
published:
created: 2026-02-24
...
这一章我们看看*几种常见的触发 `状态更新` 的方法是如何完成工作的*。
...
`render阶段` 开始于 `performSyncWorkOnRoot` 或 `performConcurrentWorkOnRoot` 方法的调用。这取决于本次更新是*同步更新*还是*异步更新*。
...
我们已经知道， render阶段 完成后会进入 commit阶段 。让我们继续补全**从 `触发状态更新` 到 render阶段 的路径**。
...
在 React 中，有如下方法可以**触发状态更新（排除 SSR 相关）：**
...
答案是：*每次 状态更新 都会创建一个保存 **更新状态相关内容** 的对象*，我们叫他 `Update` 。*在 `render阶段` 的 beginWork 中会**根据 Update 计算新的 state*** 。
