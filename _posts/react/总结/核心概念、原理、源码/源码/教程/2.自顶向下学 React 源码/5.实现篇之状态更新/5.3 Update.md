...
- `shared.pending` ：*触发更新时，产生的 Update 会保存在 shared.pending 中形成单向环状链表*。当由 Update 计算 state 时这个环会*被剪开并连接在 lastBaseUpdate 后面*。你可以将 shared.pending 类比 心智模型 中本次需要提交的 commit （节点ABC）。
