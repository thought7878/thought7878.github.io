...
- 递归模型：D阶段与归阶段  
    ...
    - D阶段（向下遍历）：执行`beginWork`
    - 归阶段（向上回溯）：执行`completeWork`
- mount与update的区别
    - mount时无对应的current Fiber树
    - update时存在current Fiber树  
...
- beginWork的核心目的  
    创建当前fiber节点的第一个子fiber节点。
- 执行流程
    - 判断当前fiber节点的tag类型 → 进入相应update逻辑
    - 根据是否存在current fiber决定是否标记副作用
    - 进入reconcile逻辑，判断children类型：
        - object → 单一React Element
        - string/number → 文本节点
        - array → 子节点数组
    - 调用`reconcileSingleElement`等具体方法创建子fiber
