---
title: React技术揭秘
source: https://react.iamkasong.com/diff/multi.html#%E6%A6%82%E8%A7%88
author:
published:
created: 2026-02-24
...
他的返回值 JSX对象 的 children 属性*不是单一节点，而是包含四个对象的数组*
...
// 之后 情况1 —— 节点属性变化
<ul>
  <li key="0" className="after">0<li> // before变成after
  <li key="1">1<li>
</ul>

// 之后 情况2 —— 节点类型更新
<ul>
  <div key="0">0</div> // li变成div
  <li key="1">1<li>
</ul>
```
...
那就是*最理想的情况*：只需在第一轮遍历进行组件 [更新](https://github.com/facebook/react/blob/1fb18e22ae66fdb1dc127347e169e73948778e5a/packages/react-reconciler/src/ReactChildFiber.new.js#L825) 。此时 Diff 结束。

### newChildren没遍历完，oldFiber遍历完（新增）

已有的 DOM节点 都复用了，这时还有新加入的节点，*意味着本次更新有新节点插入*，我们*只需要遍历剩下的 newChildren 为生成的 workInProgress fiber 依次标记 Placement* 。
...
*意味着本次更新比之前的节点数量少，有节点被删除了*。所以*需要遍历剩下的 oldFiber ，依次标记 Deletion* 。
...
*由于有节点改变了位置*，所以不能再用位置索引 i 对比前后的节点，那么*如何才能将同一个节点在两次更新中对应上呢？*

我们*需要使用 key* 。

**为了快速的找到 key 对应的 oldFiber** ，我们*将所有还未处理的 oldFiber 存入以 key 为key， oldFiber 为value的 Map 中*。
...
由于本次更新中*节点是按 newChildren 的顺序排列*。在遍历 newChildren 过程中，每个 遍历到的可复用节点 一定是当前遍历到的 所有可复用节点 中 **最靠右的那个** ，即一定在 lastPlacedIndex 对应的 可复用的节点 在本次更新中位置的后面。
...
则 b节点需要向右移动 // ！！！新的位置是3，原来的位置是1，因此b节点需要向右移动
===第二轮遍历结束===

最终acd 3个节点都没有移动，b节点被标记为移动 // ！！！b节点被标记为移动
```
