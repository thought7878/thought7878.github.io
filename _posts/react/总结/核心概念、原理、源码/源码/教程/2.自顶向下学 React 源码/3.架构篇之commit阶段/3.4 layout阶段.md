---
title: React技术揭秘
source: https://react.iamkasong.com/renderer/layout.html#commitlayouteffects
author:
published:
created: 2026-02-24
...
- 对于 `ClassComponent` ，他会**通过 current === null?区分是 mount 还是 update ，调用 [componentDidMount](https://github.com/facebook/react/blob/970fa122d8188bafa600e9b5214833487fbf1092/packages/react-reconciler/src/ReactFiberCommitWork.new.js#L538) 或 [componentDidUpdate](https://github.com/facebook/react/blob/970fa122d8188bafa600e9b5214833487fbf1092/packages/react-reconciler/src/ReactFiberCommitWork.new.js#L592)** 。
...
- 对于 `FunctionComponent` 及相关类型，他会**调用 `useLayoutEffect` hook 的 回调函数 ，调度 `useEffect` 的 销毁 与 回调 函数**
...
结合这里我们可以发现， *useLayoutEffect hook 从上一次更新的 销毁函数 调用到本次更新的 回调函数 调用是同步执行的*。*而 useEffect 则需要先调度，在 Layout阶段 完成后再异步执行。*
...
在 [双缓存机制一节](https://react.iamkasong.com/process/doubleBuffer.html#%E4%BB%80%E4%B9%88%E6%98%AF-%E5%8F%8C%E7%BC%93%E5%AD%98) 我们介绍过， workInProgress Fiber树 在 commit阶段 完成渲染后会变为 current Fiber树 。**这行代码的作用就是切换 fiberRootNode 指向的 current Fiber树** 。
