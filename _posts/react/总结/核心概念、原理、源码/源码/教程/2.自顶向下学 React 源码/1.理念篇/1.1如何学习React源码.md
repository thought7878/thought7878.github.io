...
[文字资料](https://react.iamkasong.com/)


本视频讲述了**学习React源码的难点与方法**，包括其**架构体系中的调度、协调和渲染三大模块**，并介绍了*课程设计理念、章节安排、学习React源码的价值*。
...
- `函数调用栈`可**划分为三个部分**：产生更新、决定需要更新的组件、将更新的组件渲染到页面中。
...
- `render阶段`负责**决定当前有哪些组件需要更新**。
- 深入理解JSX数据结构，render阶段划分为**递与归两个子阶段**。

### 第三章：commit阶段（渲染器的执行流程）
...
- commit阶段负责**将需要更新的组件渲染到视图中**。
- 划分为**渲染前、渲染中、渲染后三个子阶段**。
...
- update优先级机制。
- ReactDOM.render与this.setState完整执行流程。
