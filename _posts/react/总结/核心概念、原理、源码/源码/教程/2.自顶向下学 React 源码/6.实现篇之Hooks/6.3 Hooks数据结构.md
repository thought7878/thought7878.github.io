---
title: React技术揭秘
source: https://react.iamkasong.com/hooks/structure.html#memoizedstate
author:
published:
created: 2026-03-11
...
并*将不同情况对应的 dispatcher 赋值给`全局变量 ReactCurrentDispatcher` 的 current 属性*。
...
> 在`renderWithHooks`中，你可以在 [这里](https://github.com/acdlite/react/blob/1fb18e22ae66fdb1dc127347e169e73948778e5a/packages/react-reconciler/src/ReactFiberHooks.new.js#L409) 看到这行代码
...
- `hook.memoizedState` ： Hooks 链表中*保存的单一 hook 对应的数据*。
...
- `useCallback`：对于 useCallback(callback, [depA]) ， *memoizedState 保存 [callback, depA]* 。*与 useMemo 的区别*是， useCallback 保存的是 callback 函数本身，而 useMemo 保存的是 callback 函数的执行结果
