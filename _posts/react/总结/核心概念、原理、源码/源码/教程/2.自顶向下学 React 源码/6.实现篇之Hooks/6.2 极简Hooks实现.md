---
title: React技术揭秘
source: https://react.iamkasong.com/hooks/create.html#%E5%AF%B9%E8%A7%A6%E5%8F%91%E4%BA%8B%E4%BB%B6%E8%BF%9B%E8%A1%8C%E6%8A%BD%E8%B1%A1
author:
published:
created: 2026-03-11
...
当产生*第一个 update* （我们叫他 u0 ），此时 queue.pending === null 。
...
当产生*第二个 update* （我们叫他 u1 ）， update.next = queue.pending.next;，此时 queue.pending.next === u0 ， 即 u1.next = u0 。
...
你可以照着这个例子模拟插入多个 update 的情况，会发现 **queue.pending 始终指向最后一个插入的 update** 。**这样做的好处是**，当我们要遍历 update 时， queue.pending.next 指向第一个插入的 update 。
...
通过 `workInProgressHook` 变量*指向当前正在工作的 hook* 。
...
[关注公众号 魔术师卡颂](https://react.iamkasong.com/me.html) ，后台回复 **616** 获得在线 Demo 地址
