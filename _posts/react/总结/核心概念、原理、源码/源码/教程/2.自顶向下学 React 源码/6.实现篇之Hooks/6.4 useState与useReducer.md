---
title: React技术揭秘
source: https://react.iamkasong.com/hooks/usestate.html#%E6%B5%81%E7%A8%8B%E6%A6%82%E8%A7%88
author:
published:
created: 2026-03-11
...
我们*将这两个 Hook 的工作流程分为 `声明阶段` 和 `调用阶段`* ，对于：
...
function mountState<S>(
  ...
  const queue = (hook.queue = {
    pending: null,
    dispatch: null,
    // 对比useReducer，只有这里不同（默认的reducer）
...
function mountReducer<S, I, A>(
  ...
  const queue = (hook.queue = {
    pending: null,
    dispatch: null,
    // 对比useState，只有这里不同（用户自定义的reducer）
...
可以看到， **mount 时这两个 Hook 的唯一区别**为 queue 参数的 lastRenderedReducer 字段。
...
mount 时获取当前 hook 使用的是 `mountWorkInProgressHook` ，而 update 时使用的是 `updateWorkInProgressHook` ，这里的*原因是：*

- `mount` 时可以确定是调用 ReactDOM.render 或相关初始化 API 产生的 更新 ，*只会执行一次*。
- `update` 可能是*在事件回调或副作用中触发的 更新* 或者是 *render阶段 触发的 更新* ，**为了避免组件无限循环 更新 ，后者需要区别对待**。
...
获取对应 hook ，接下来会根据 hook 中保存的 state 计算新的 state ，这个步骤同 [Update 一节](https://react.iamkasong.com/state/update.html) 一致。
...
触发 更新 时*通过 bind 预先保存的 fiber 与 workInProgress 全等，代表本次 更新 发生于 FunctionComponent 对应 fiber 的 render阶段* 。**所以这是一个 render阶段 触发的 更新** ，需要标记变量 didScheduleRenderPhaseUpdate ，后续单独处理。
...
[关注公众号 魔术师卡颂](https://react.iamkasong.com/me.html) ，后台回复 **582** 获得在线 Demo 地址