...
![[_posts/react/总结/核心概念、原理、源码/源码/教程/React18底层源码深入剖析/第5章 React18全新底层核心运行机制：任务调度机制的代码实践/media/f753862c5c228185bf13ced22c4621c8_MD5.webp]]
```ts
...
- 作用说明  
    `sortIndex` 是**任务在最小堆中的排序依据，决定任务执行优先级**。
- 取值逻辑  
    **使用 expiration time 作为 sortIndex 的值**。
- 原因分析  
    *最小堆保证 sortIndex 最小的任务最先执行*。*越早过期（即 expiration time 越小）的任务越优先，符合调度目标*。
...
- **全局锁变量** [09:29](https://b.quark.cn/apps/5AZ7aRopS/routes/quark-video-ai-summary/pc?debug=0&fid=649abdd1fba04df493057be1e8facc1d#?seek_t=569)  
    定义布尔变量 `isHostCallbackScheduled`，用于*标识主线程是否已被调度占用*。
- **加锁机制**  
    在触发调度前将 isHostCallbackScheduled设为 true，防止重复调度。
...
- 关键点回顾
    ...
    - 使用 `expiration time` 作为 `sortIndex` 以支持最小堆排序。
