...
![[6d6428a5a05a20c1b37ef401a1ea3711_MD5.png]]
...
![[776f0303479600502d8da91c8eb3743f_MD5.png]]

在 index.tsx 里把这三行代码注释掉：

![[67d40f9654a2acfd199fd27b3324d47d_MD5.png]]
...
![[1b556363ea7f1e2897a5b25f335f0634_MD5.png]]
...
![[9b19ebc0bac5ff73c67c42de4a4d39f5_MD5.gif]]

state 初始值是 1，点击改变状态，这个很简单。

如果*初始状态需要经过复杂计算得到*，可以传个函数来计算初始值：

![[790efcf50a9b892a057d61bfd14dd6a9_MD5.png]]
...
![[f1775b02dc3927c358112626e55caa6f_MD5.png]]

可以直接传新的值，或者传一个函数，返回新的值，这个函数的参数是上一次的 state。

跑一下，功能正常：

![[f8c363e8dc7f16025c81cdd58d49ef89_MD5.gif]]
...
![[1cf4d34abbf72c8202328b1245cd0a01_MD5.png]]
...
![[646002a3800b3a9a755680fdc93f4ef7_MD5.png]]

浏览器访问下，可以看到 2s 后，state 变为了 666:

![[e745de01e4aebbb09c1a3108df573ab6_MD5.gif]]

请求数据、定时器等这些异步逻辑，我们都会放在 useEffect 里。

回过头来看下 useEffect：

![[6591f2b7182758f8c3b4c92cf802abcb_MD5.png]]
...
![[0670e89b30eedbb872eb1eb5389d314f_MD5.png]]

现在这个组件会渲染两次，初始渲染和 2s 后 setNum 触发的渲染，也就是 function 会执行 2 次。

打开 devtools 看一下：

![[e2506d0c3156d27ee8a2507d14343a70_MD5.png]]

xxx 只执行了一次，因为 [] 每次都不变。

我也可以写任意的常量，因为它们都不变，所以不会触发 effect 的重新执行：

![[9f9086805b83716e1e068a45914e791d_MD5.png]]

![[2bb5e75fac5ff72b5e7ec85ef157abb4_MD5.png]]

但如果其中有个变化的值，那就会触发重新执行了：

![[abe825d1e6afda9a60ab6125b4272908_MD5.png]]

可以看到，现在每次渲染，effect 都会执行：

![[f8031e6e75ed959d033729e1d9e772cd_MD5.png]]

这个数组我们一般写依赖的 state，这样在 state 变了之后就会触发重新执行了。

不传 deps 数组的时候也是每次都会重新执行 effect 函数：

![[6630dc94fe025a4de8924dd6b1c25c63_MD5.png]]

![[1bafb35c4f1331b0990cc7b7045c5f9a_MD5.png]]

那 useEffect 里如果跑了一个定时器，依赖变了之后，再次执行 useEffect，又跑了一个，怎么清理上一个定时器呢？

这样写：

![[267b3d66516acd6907ca7ea6de390b14_MD5.png]]
...
![[54314325fdf48f771bfd226469d15528_MD5.gif]]
...
![[47b55589f44dfb8e46966347ab75347a_MD5.png]]

![[c0538e0f70a0a1c0b88c2e8ab3b8c17f_MD5.png]]

那为啥还要有这两个 hook 呢？

我们知道，js 执行和渲染是阻塞的：

![[8de62aed0505bcc263351051e5059e28_MD5.png]]

useEffect 的 effect 函数会在操作 dom 之后异步执行：

![[ad1e30503a42eb407855b93205aa7021_MD5.png]]
...
![[85a3a93876868fbf1ed17235efebb1cb_MD5.png]]

可以看到，渲染的间隔是固定的，而 js 的任务在这些渲染的间隔中执行。

所以异步执行的 effect 逻辑就有两种可能：

![[f7505ece6a7511a1ff749cb746103b5d_MD5.png]]

![[ba1ae0f22519e21a25cbed8fd6c553c2_MD5.png]]
...
![[4c8de131f818b7d5c4fe9fc3d5e60907_MD5.png]]
...
![[059dfe64ced73b681bd471df0dee91e6_MD5.png]]
...
![[0faa2a46b8e89bb2888370b4456c2fd8_MD5.png]]

![[239bf1968e0b80293076447b77726c8d_MD5.gif]]
...
![[be5c30ed0ad7a1517ae71411d9bba0dc_MD5.png]]
...
![[1642b7736c68eb09cfaca6e809baa0df_MD5.png]]

如果你直接修改原始的 state 返回，是触发不了重新渲染的：

![[173e52228d40c1f8a0cabc8e2a11c749_MD5.gif]]
...
![[363134dc3a3a5ae7dcdb52d8dde61a87_MD5.gif]]
...
![[fbc21404fe771bb5de747b465617b6a5_MD5.png]]
...
![[b717c6ba083653784148f0907b26f647_MD5.gif]]
...
![[acbfecf9f557262d70e427dd995bfd56_MD5.gif]]

也可以用 immer：

![[505e889927d8ab7122096e18a2988694_MD5.png]]
...
```
![[6c537a6145a486ad2f7d5fd75cc53a7a_MD5.gif]]
...
![[1a188bd3f14d7c64dec8812238a600c7_MD5.png]]

可以看到，focus 生效了：

![[da5cc5c671527aa1dece3e97ac4f07c0_MD5.gif]]
...
![[7e610151aa7d5639a6610ea6c6105497_MD5.gif]]

想触发渲染，还是得配合 state：

![[970481f5e7543fec82befe0f38e0dc3c_MD5.png]]
...
![[81bf9d18eebf9dfb592607dbd30ef9b5_MD5.gif]]
...
![[6ae202f1794c59e03dc491ecaba655db_MD5.png]]
...
![[cfb2d4fb75c50e8cc488ef604b03df3b_MD5.png]]

这样就把子组件的 input 的 ref 传递到了父组件。

效果如下：

![[9de843837c48543b79a7a61607af67d1_MD5.png]]

不过被 forwardRef 包裹的组件的类型就要用 React.forwardRefRenderFunction 了：

![[123af501a2d35dffc5e67ba429a31d5c_MD5.png]]
...
![[a841341265ca32c0c8c56e9a4049380b_MD5.png]]

这样，父组件里拿到的 ref 就是 useImperativeHandle 第二个参数的返回值了。

效果是一样的：

![[66da89c25562937bf2f435ed70c7f3ab_MD5.png]]

## useContext

跨任意层组件传递数据，我们一般用 Context。

![[c4a8257995bbd0e84d381bd1c8010ede_MD5.png]]

创建 App5.tsx

![[65538463d27fc8c7b85ac5173ddbd610_MD5.png]]
...
![[2d320a8afe86ac0afadc77b37aa20375_MD5.png]]

访问下：

![[e6764cd0514de09d254c6a1be0b2ee1b_MD5.png]]
...
![[2935ce67f1dabd471f967e18b3e89e24_MD5.png]]
...
![[c33b71b361efadb9de7c65b5368500c5_MD5.png]]
...
![[0da60127485b296e07eebe0c08707c51_MD5.png]]
...
![[210b832cb750eaeaa2d747eb1edbd5ce_MD5.png]]

我们让 2s 后 props 变了呢？

![[5bf8f8d246bb76d246d353a52fca7f6f_MD5.png]]
...
![[01a07c0f598ad8299455461cd9afcc4d_MD5.png]]
...
![[4ed9408b7ebd93baefca34a8e6db471d_MD5.png]]

参数传了一个 function，你会发现 memo 失效了：

![[3eb37e5fca4d258f1d1c4ae175c5ec97_MD5.png]]

因为每次 function 都是新创建的，也就是每次 props 都会变，这样 memo 就没用了。

这时候就需要 useCallback：

![[dd76ec765aa523e2d159fdf8afc9bfa2_MD5.png]]
...
![[8d2d70d224274ff4ccde5992c82d2d9b_MD5.png]]

同理，useMemo 也是和 memo 打配合的，只不过它保存的不是函数，而是值：

![[103768bc339ff0f61876e8d1d437271e_MD5.png]]
