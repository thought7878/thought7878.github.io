...
## Grid网格划分2：fr单位的用法
- `{{video(https://www.bilibili.com/video/BV1i6421c7Ue/?spm_id_from=333.788&vd_source=22af953ea4c09540ad1966711a2d53f0)}}`

- `{{renderer(:media-timestamp,02:55)}}` 代码演示

## Grid网格划分3：auto和minmax()
- `{{video(https://www.bilibili.com/video/BV1SS421w7dK/?spm_id_from=333.788&vd_source=22af953ea4c09540ad1966711a2d53f0)}}`

- `{{renderer(:media-timestamp,00:00)}}` auto：有fr时，单元格的尺寸是元素的尺寸；没有fr时，沾满剩余空间
- `{{renderer(:media-timestamp,04:09)}}` minmax()：最小min，最大max。尽可能占用空间直到max，占用空间的优先级>fr>auto

- `{{renderer(:media-timestamp,05:58)}}` 演示

## Grid网格划分4：auto/fr/minmax的三个要点
- `{{video(https://www.bilibili.com/video/BV1di421d7x8/?spm_id_from=pageDriver&vd_source=22af953ea4c09540ad1966711a2d53f0)}}`

- `{{renderer(:media-timestamp,00:00)}}`空间占用优先级：minmax()>fr>auto
- `{{renderer(:media-timestamp,01:36)}}`最大空间和最小空间
- `{{renderer(:media-timestamp,03:37)}}`为了保证最小空间，网格可能会超出容器的范围

## Grid网格划分5：repeat和gap
- `{{video(https://www.bilibili.com/video/BV1SD42177Hs/?spm_id_from=pageDriver&vd_source=22af953ea4c09540ad1966711a2d53f0)}}`

- `{{renderer(:media-timestamp,00:00)}}`repeat()

- `{{renderer(:media-timestamp,02:03)}}`auto-fill():根据尺寸尽可能多地动态地生成行列数
- `{{renderer(:media-timestamp,03:15)}}`auto-fit():内部没有元素，单元格尺寸收缩为0

- `{{renderer(:media-timestamp,05:05)}}`gap：column-gap/row-gap
