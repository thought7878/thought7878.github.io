JavaScript 的函数式编程（Functional Programming，FP）是一种强调纯函数、不可变数据和高阶函数的编程范式。尽管 JavaScript 是多范式语言，但其对一等函数（First-Class Functions）的支持使其非常适合函数式编程。以下是核心概念、实践方法和应用场景的系统性总结：
