`WeakMap` 是 JavaScript 中的一种*特殊 Map 类型，它**只接受对象作为键**，并且这些键是弱引用的*。这意味着 `WeakMap` 的键**不会阻止垃圾回收机制**，***如果一个对象仅被 `WeakMap` 引用（即没有其他强引用），那么这个对象可能会被垃圾回收器回收***。这使得 `WeakMap` 特别适合用于**存储那些不应该影响对象生命周期的数据**。
