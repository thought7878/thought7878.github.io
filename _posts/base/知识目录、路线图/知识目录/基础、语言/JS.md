...
## 深拷贝
参考：[[_posts/base/js/知识点总结/数据类型/深拷贝|深拷贝]]
...
参考：[[Promise]]、[[JavaScript 可视化 - Promise 执行（双语字幕）|JavaScript 可视化 - Promise 执行（双语字幕）]]、[[手写 Promise]]
...
参考：[[_Event Loop, Web APIs, (Micro)task Queue]]、[[异步原理（Event Loop）]]
