...
XSS 简单点来说，就是**攻击者想尽一切办法将可以执行的代码注入到网页中**。
...
首先，**对于用户的输入应该是永远不信任的**。最普遍的做法就是**转义输入输出的内容，对于引号、尖括号、斜杠进行转义**
...
CSRF 中文名为跨站请求伪造。**原理**就是攻击者构造出一个后端请求地址，诱导用户点击或者通过某些途径自动发起请求。如果用户是在登录状态下的话，后端就以为是用户在操作，从而进行相应的逻辑。
