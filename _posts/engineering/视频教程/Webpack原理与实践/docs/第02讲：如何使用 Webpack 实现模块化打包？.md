...
Webpack 4 以后的版本*支持零配置*的方式直接启动打包，*整个过程会按照约定将 src/index.js 作为打包入口，最终打包的结果会存放到 dist/main.js 中*。
...
我们知道， VSCode 对于代码的自动提示是根据成员的类型推断出来的，换句话说，*如果 VSCode 知道当前变量的类型，就可以给出正确的智能提示*。即便你没有使用 TypeScript 这种类型友好的语言，也可以*通过类型注释的方式去标注变量的类型*。
