...
这里我们先*移除 import 这种静态导入*，然后*在需要使用组件的地方通过 import 函数导入指定路径*，那这个方法返回的是一个 Promise。在这个 Promise 的 then 方法中我们能够拿到模块对象。由于我们这里的 posts 和 album 模块是以默认成员导出，所以我们需要解构模块对象中的 default，先拿到导出成员，然后再正常使用这个导出成员。
