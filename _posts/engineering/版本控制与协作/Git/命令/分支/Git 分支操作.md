...
| 问题 | 核心答题要点 |
|------|--------------|
| 分支和标签的本质区别？ | 都是引用指针。分支在 `refs/heads/`，随 commit 动态前移；标签在 `refs/tags/`，创建后静态固定。分支用于开发流，标签用于版本快照。 |
| `git checkout` 和 `git switch` 区别？ | `checkout` 是历史遗留的多功能命令（切分支+恢复文件+进 detached）；`switch` 专注分支操作，语义清晰，误操作少。现代团队推荐 `switch` + `restore`。 |
