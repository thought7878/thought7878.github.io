[tsconfig.json](file:///Users/ll/Desktop/learn-notes/frontend/react/3rd/react-day-picker/tsconfig.json) 是 TypeScript 项目的核心配置文件，用于定义项目的*编译选项、路径映射、类型检查规则*等内容。它告诉 TypeScript 编译器（`tsc`）如何处理项目中的 `.ts` 和 `.tsx` 文件。
