...
自动部署（又叫持续部署 Continuous Deployment，英文缩写 CD）一般有<u>两种触发方式：</u>
...
<u>这种方式不太好，很有可能软件刚部署完开发就改代码了。为了看到新的页面效果，不得不等到下一次构建开始</u>。<u>另外还有一个副作用</u>，假如开发一天都没更改代码，构建软件还是会不停的执行打包、部署操作，白白的浪费资源。
...
为了满足不同用户的需要，本章将使用 [Jenkins ](https://www.jenkins.io/zh/)和[GitHub Actions](https://docs.github.com/en/actions)来讲解如何部署前端项目。
...
2. 打开 [Jenkins ](https://www.jenkins.io/zh/download/)下载页面。 ![在这里插入图片描述](https://img-blog.csdnimg.cn/img_convert/589872cde9a22a23e5a5361e70421d22.png)
...
请参考[创建SSH密钥对 ](https://www.alibabacloud.com/help/zh/doc-detail/51793.htm)和[绑定SSH密钥对 abacloud.com/help/zh/doc-detail/51796.htm?spm=a2c63.p38356.879954.9.cf992580IYf2O7#concept-zzt-nl1-ydb)，将你的 ECS 服务器实例和密钥绑定，然后将私钥保存到你的电脑（例如保存在 ecs.pem 文件）。
...
`steps` 其实是一个数组，在 YAML 语法中，以 `-` 开始就是一个数组项。例如 `['a', 'b', 'c']` 用 YAML 语法表示为：
