...
# 重点内容
- 1.GitLab CI/CD 简介、CI/CD 的基本概念、 DevOps 理念
	- GitLab CI/CD **课程介绍**
	- 什么是 GitLab CI/CD？
	- 
...
第二部分（25 讲）——使用 Docker 构建环境进行持续集成、自动化测试、并行执行、JUnit 报告生成、合并请求工作流程、代码检查、工件发布。
...
## 1.GitLab CI/CD 简介、CI/CD 的基本概念、 DevOps 理念
01 - Introduction to GitLab CICD. Fundamental CICD concepts & DevOps
22 场讲座 • 1 小时 50 分钟
...
*通过构建一条流水线来探索 GitLab CI/CD 的基础知识*。这条流水线*能够构建、测试简单的网站*，并*将其部署到 AWS 上*。同时，该系统*还融合了自动化、DevOps 最佳实践、生成式人工智能的功能*。
...
可以*将管道系统视为一条汽车装配线*：**遵循一系列有序且相互连接的步骤，其中一些步骤是并行进行的，最后进行测试以确保软件产品的质量**。
...
本次讲座旨在让您**了解什么是管道机制**，以及*如何将管道机制集成到 Gitlab CI 中*。我们将*通过一个简单的示例来说明这一过程*。具体来说，我们*将分为两个简单的阶段，并在每个阶段定义两个任务，这些任务将被分配给 Gitlab CI 中的各个阶段*。
...
创建一个*包含构建阶段和测试阶段的双阶段* GitLab CI 管道系统。*将 car.txt 文件作为人工制品保存下来*，并通过 cat、test 和 grep 工具来验证其存在以及内容是否正常。
...
![[_posts/devops/课程/GitLab CICD：面向初学者的管道构建、持续集成、持续部署及 DevOps 相关知识/media/244151e2b36761bc7523f0da0c77c0eb_MD5.webp]]

[[GitLab CICD 架构]]

#### GitLab CI/CD 架构（第二部分）
...
了解 GitLab CI/CD 是*如何使用 Docker 执行器来运行带有默认或指定镜像的作业的*。*了解有关产物/制品、构建目录、隔离环境的详细信息*，这些功能**使得管道能够更加灵活且可并行处理任务**。
...
请*比较使用 GitLab.com 管理以及自行管理部署的情况*。免费计划提供 400 分钟的计算资源，同时还有附加功能，以及*可以在自己的基础设施上运行运行器的选项*。

### 架构测试题
GitLab architecture quizGitLab
...
了解如何*通过 Linux 命令来控制 GitLab 的流水线作业*，使用命令行界面和 shell 脚本实现自动化操作，取代传统的图形化界面，同时注重命令使用的正确性。
...
掌握用于 GitLab CI/CD 的 *YAML 语法基础*，包括*正确使用空格进行缩进、正确的冒号使用方式、列表格式规范*等。同时*了解常见的 YAML 语法错误及其解决方法*。

### *通过有效的测试*来保护 CI pipelines
...
通过添加发动机和车轮来改进汽车组装流程。确保测试能够验证底盘、发动机和车轮的正常工作，并学会*通过日志来诊断测试中的故障*。

### 为什么jobs和pipelines会失败（退出代码）
...
了解退出码如何决定 GitLab CI/CD 的运作方式：*0 表示成功，非 0 则意味着任务失败，流水线也会停止*。此外，这里还提供了*一些调试技巧，并给出了一个退出码为 1 的示例*。
...
## 2.GitLab的CI（阶段、测试、报告、合并请求）
02 - Continuous Integration (CI) with GitLab (Stages, Tests, Reports, Merge Requests)
25 场讲座 • 每场 2 小时
...
### Forking a Git repository分割一个 Git 仓库4:23
...
### GitLab architecture quizGitLab 架构测试题
...
### Running jobs in parallel并行运行任务3:00
...
### Simplifying the pipeline简化流程3:25
...
## 3.GitLab 的CD
03 - Continuous Deployment (CD) with GitLab
基于 GitLab 的持续部署系统
34 次讲座 • 2 小时 58 分钟

### Section Overview部分概述2:50
...
### Installing CLI tools安装 CLI 工具5:19
...
### Deploying to production部署到生产环境4:26
...
### Deployments with GitLab使用 GitLab 进行部署
...
### Setting a build version设置构建版本5:31
...
### Why CI/CD really matters为什么 CI/CD 真的如此重要6:15
...
## 4.面向 DevOps 的 Docker 入门指南
04 - Introduction to Docker for DevOps
9 次讲座 • 每次 51 分钟

### Section overview部分概述1:51

### Creating a Dockerfile创建 Dockerfile2:57
...
### Building a Docker image构建 Docker 镜像4:58
...
## 5.CD 到 AWS （使用 AWS CLI、IAM、S3 等工具）、结合 GitLab。
05 - Continuous Deployment to AWS (AWS CLI, IAM, S3) with GitLab
...
### Syncing files to S3将文件同步到 S3 存储服务4:20
...
## 6.通过 SSH 部署到远程服务器（使用 SSH、SCP、rsync 等工具）。
06 - Deploying over SSH to a remote server with GitLab (SSH, SCP, rsync)
...
## 7.持续部署至 Amazon Elastic Container Service（ECS、ECR、IAM）平台
07 - Continuous Deployment to Amazon Elastic Container Service (ECS, ECR, IAM)
...
### Creating the Dockerfile创建 Docker 文件2:56
...
## 8.高级的 GitLab CI/CD 功能
08 - Advanced GitLab CICD features
12 次讲座 • 1 小时 17 分钟

### Section overview部分概述1:07
...
### YAML anchors and aliasesYAML 语法中的锚点及别名9:24
...
## 9.Pipeline性能优化
09 - Pipeline performance optimizations
12 场讲座 • 每场 48 分钟
...
## 10.具体主题/用户提出的主题
10 - Specific topics  User topics
1 次讲座 • 1 分钟
...
### Not the end还不是结束0:21

### Bonus lecture额外讲座0:22
...
**WORKPLACE APPLICATIONS工作场所应用程序**
...
- Solutions architects解决方案架构师
- Application Architects应用架构师
