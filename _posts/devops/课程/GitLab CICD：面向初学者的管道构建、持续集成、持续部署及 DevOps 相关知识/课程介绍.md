...
*通过调试流水线、分析日志以及实践对系统进行逐步修改的方法*来，*培养必要的 DevOps 和 GitLab CI/CD 故障排除技能*。
...
将 `DevOps` 定义为**一种以文化为驱动的协作方式，而非某种标准或工具**。同时展示*如何通过自动化和持续集成/持续交付机制来实现持续的反馈机制，以及更快速、更可靠的软件交付*。
...
### 重新审视 GitLab CI/CD 架构
Revisiting the GitLab CI/CD architecture
3:11

### GitLab 架构测试题
GitLab architecture quiz
...
### 默认的分管阶段（.pre、build、test、deploy .post）
...
### 脚本：before_script 和 after_script
...
### 25 - 测试从 GitLab 进行的 SSH 连接
25 - Testing the SSH connection from GitLab
5:15

### 将 SSH 私有密钥存储在 GitLab 中
...
### ECS 基础设施层/启动模式（Fargate 与 EC2 对比）
...
### 使用“needs”这个关键词与“artifacts”进行关联
